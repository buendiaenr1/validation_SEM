...
# ==============================================================================
print("\n" + "=" * 70)
print("4. IDENTIFICACIÓN DE ÍTEMS INVERSOS")
print("=" * 70)
...
if NUM_FACTORES_AFE is None:
    # Usar el codo del scree plot o Kaiser, el menor de los dos
    # Buscar el "codo" en el scree plot
    ...
    n_factors_aft = min(n_factors_kaiser, n_factors_scree, 10)  # Máximo 10 factores
    n_factors_aft = max(n_factors_aft, 1)  # Mínimo 1 factor
else:
    n_factors_aft = NUM_FACTORES_AFE

print(f"🔹 Número de factores a extraer: {n_factors_aft}")

# Scree Plot
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

axes[0].plot(range(1, len(eigenvalues) + 1), eigenvalues, 'bo-', linewidth=2, markersize=6)
axes[0].axhline(y=1, color='r', linestyle='--', label='Autovalor = 1')
...
try:
    ...
    print(f"   CFI:   {stats_1f['CFI'].values[0]:.4f}")
    print(f"   RMSEA: {stats_1f['RMSEA'].values[0]:.4f}")
    print(f"   TLI:   {stats_1f['TLI'].values[0]:.4f}")
...
if n_factors_aft > 1:
    ...
    if len(asignacion_valida) > 1:
        ...
        try:
            ...
            print(f"   CFI:   {stats_mf['CFI'].values[0]:.4f}")
            print(f"   RMSEA: {stats_mf['RMSEA'].values[0]:.4f}")
            print(f"   TLI:   {stats_mf['TLI'].values[0]:.4f}")
...
# ==============================================================================
# 7. ÍNDICES DE AJUSTE DEL MODELO
# ==============================================================================
print("\n" + "=" * 70)
print("7. ÍNDICES DE AJUSTE DEL MODELO")
print("=" * 70)

def interpretar_ajuste(cfi, rmsea, tli, gfi):
    """Interpreta los índices de ajuste."""
    resultados = {}
    
    resultados['CFI'] = 'Excelente' if cfi >= 0.95 else 'Bueno' if cfi >= 0.90 else 'Aceptable' if cfi >= 0.80 else 'Pobre'
    resultados['RMSEA'] = 'Excelente' if rmsea <= 0.05 else 'Aceptable' if rmsea <= 0.08 else 'Pobre'
    resultados['TLI'] = 'Excelente' if tli >= 0.95 else 'Bueno' if tli >= 0.90 else 'Aceptable' if tli >= 0.80 else 'Pobre'
    resultados['GFI'] = 'Excelente' if gfi >= 0.95 else 'Bueno' if gfi >= 0.90 else 'Aceptable' if gfi >= 0.80 else 'Pobre'
...
print("   CFI ≥ 0.90: Buen ajuste    |  RMSEA ≤ 0.05: Buen ajuste")
print("   TLI ≥ 0.90: Buen ajuste    |  GFI ≥ 0.90: Buen ajuste")
...
if len(modelos_stats) > 1:
    mejor_modelo = min(modelos_stats.items(), key=lambda x: x[1]['AIC'])
    nombre_mejor = mejor_modelo[0]
else:
    nombre_mejor = "Unidimensional"
...
│     • Factores según Kaiser: {n_factors_kaiser}{' ' * (27 - len(str(n_factors_kaiser)))}
│                                                                     
│  🔧 RECODIFICACIÓN:                                                  
│     • {len(items_inversos)} ítems inversos identificados{' ' * (29 - len(str(len(items_inversos))))}
│                                                                     
│  📈 ANÁLISIS FACTORIAL:                                              
...
│                                                                     
│  📐 MEJOR MODELO SEM: {nombre_mejor}{' ' * (38 - len(nombre_mejor))}
│"""

if nombre_mejor in modelos_stats:
    stats = modelos_stats[nombre_mejor]
    resumen += f"""
│     • CFI = {stats['CFI']:.3f}                                              │
